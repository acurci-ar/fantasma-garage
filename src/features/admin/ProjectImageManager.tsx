...
import { reorderProjectImages } from "@/actions/admin/projects";
import type { ProjectImage, ProjectStage } from "@/types/database";
import { useDragReorder } from "@/lib/utils/useDragReorder";
import { cn } from "@/lib/utils/cn";

function sortByPosition(images: ProjectImage[]): ProjectImage[] {
  return [...images].sort((a, b) => a.position - b.position);
}

/** Lista de fotos del proyecto, arrastrable (ícono de agarre a la izquierda de cada fila) para reordenar. */
      ...
      {ordered.map((image, index) => (
        ...
        >
          ...
          <div className="flex-1">
            <ProjectImageRow image={image} projectId={projectId} stages={stages} />
          </div>
        </div>
      ))}
    </div>
