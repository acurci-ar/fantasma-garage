...
import { CarImageRow } from "@/features/admin/CarImageRow";
import { reorderCarImages } from "@/actions/admin/cars";
import type { CarImage } from "@/types/database";
import { useDragReorder } from "@/lib/utils/useDragReorder";
import { cn } from "@/lib/utils/cn";

function sortByPosition(images: CarImage[]): CarImage[] {
  return [...images].sort((a, b) => a.position - b.position);
}

/** Lista de fotos del auto, arrastrable (ícono de agarre) para reordenar — mismo patrón que ProductImageManager/ProjectImageManager. */
export function CarImageManager({ images, carId }: { images: CarImage[]; carId: string }) {
      ...
      {ordered.map((image, index) => (
        ...
        >
          ...
          <div className="flex-1">
            <CarImageRow image={image} carId={carId} />
          </div>
        </div>
      ))}
    </div>
